...
import { autoContinue, setAutoRun } from './autorun.js';
import { tryGetKeyThen } from './keyfetch.js';
import { enterPoolFor, followEntryHint, poolIndexFor, POOL_ORDER } from './pools.js';
...
export function awaitAreaThen(areaName, fn, tries){
...
export const HUNT_UNABLE   = /unable\s+to\s+hunt\s+that\s+target|seem\s+unable\s+to\s+hunt/i;
export const HUNT_IS_HERE  = /\bis here\b/i;
...
export const WHERE_ROW = /^(.{1,30}?)\s{2,}([^ (0-9].*?)\s*$/;
...
// -----------------------------------------------------------------------------
export const HUNT_DIRS = {north:'n', south:'s', east:'e', west:'w', northeast:'ne',
                   northwest:'nw', southeast:'se', southwest:'sw', up:'u', down:'d'};
export const HUNT_DIR_RE = /(?:is|heading|leading|fled|went|go|toward)\s+(?:the\s+)?(north(?:east|west)?|south(?:east|west)?|east|west|up|down)\b/i;
